...
class MambaMT(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if hasattr(self.model, "lm_head"):
            print('Replacing lm_head with tgt_vocab_size')
            del self.model.lm_head
        
        self.lm_head = torch.nn.Linear(d_model, tgt_vocab_size, bias=False, **factory_kwargs)
    ...
    # Forward wrapper with difference lm_head
    def forward(self, input_ids, position_ids=None, inference_params=None, num_last_tokens=0, **mixer_kwargs):
        """
        "position_ids" is just to be compatible with Transformer generation. We don't use it.
        num_last_tokens: if > 0, only return the logits for the last n tokens
        """
        hidden_states = self.model.backbone(input_ids, inference_params=inference_params, **mixer_kwargs)
    ...
    def encode(self, src, source_attention_mask):
        memory = self.model.backbone.forward(
            input_ids=src,
