...
# https://github.com/HazyResearch/based/blob/main/based/models/mixers/slide_attention.py
...
class FlashSelfAttention(nn.Module):
    ...
    def __init__(
        self, causal=True, softmax_scale=None, attention_dropout=0.0, window_size=None
    ):
        super().__init__()
        assert (
...
def _update_kv_cache(kv, inference_params, layer_idx):
    ...
    assert batch_end <= (
        kv_cache.shape[0] if kv_cache is not None else kv_cache.shape[0]
    )
    assert sequence_end <= (
        kv_cache.shape[1] if kv_cache is not None else kv_cache.shape[2]
    )
...
class SlidingAttentionWrapper(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attention = SlidingAttention(
            embed_dim=d_model,
            num_heads=n_head,
            dropout=dropout,
            causal=True,
            ...
            rotary_emb_dim=(d_model // n_head),  # from based code
