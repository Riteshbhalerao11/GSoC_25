...
try:
    ...
    print(f'Import Nhi hora h bhai {i}')
    RMSNorm, layer_norm_fn, rms_norm_fn = None, None, None


class FlashSelfAttentionWrapper(nn.Module):
    ...
    def forward(
        ...
        hidden_states, cache = self.attention.forward(
            x=hidden_states, mask=mask, return_intermediates=True, cache=cache
        )

        if inference_params is not None:
            inference_params.key_value_memory_dict[self.layer_idx] = cache

            # attention concats the cache so we need to slice it
            hidden_states = (
                hidden_states[:, -1:, :]
